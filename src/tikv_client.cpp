...
config get_default_config() {
  ...
  conf.key_path = "";
  conf.timeout = 10000;
  conf.tcp_keepalive = 10000;
  conf.keep_alive_timeout = 3000;
  conf.dns_server_addr = "";
  conf.dns_search_domain = rust::cxxbridge1::Vec<rust::cxxbridge1::String>();
  return conf;
}
...
RawKVClient::RawKVClient(const std::vector<std::string> &pd_endpoints,
                         const config &conf)
    : _client(tikv_client_glue::raw_client_config(pd_endpoints, conf)) {}
